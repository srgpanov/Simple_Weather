...
import com.srgpanov.simpleweather.data.entity.Sunrise
...
class WeatherDetailAdapter() : RecyclerView.Adapter<WeatherDetailHolders>() {
    ...
    fun setData(request: WeatherRequest) {
    ...
    private fun getDataListFromRequest(request: WeatherRequest): MutableList<Any> {
        ...
        for (hours in request.forecasts[0].hours) {
            if ((hours.hour >= request.getServerHour())) {
                ...
                if (hours.hour == todaySunrise) listFromRequest.add(
                    Sunrise(request.forecasts[0].sunrise, true, request.forecasts[1].date)
                )
                if (hours.hour == todaySunset) listFromRequest.add(
                    Sunrise(request.forecasts[0].sunset, false, request.forecasts[1].date)
                )
            }
        }
        for (hours in request.forecasts[1].hours) {
            if ((hours.hour <= request.getServerHour())) {
                ...
                if (hours.hour == tomorrowSunrise) listFromRequest.add(
                    Sunrise(request.forecasts[1].sunrise, true, request.forecasts[1].date)
                )
                if (hours.hour == tomorrowSunset) listFromRequest.add(
                    Sunrise(request.forecasts[1].sunset, false, request.forecasts[1].date)
                )
            }
        }
